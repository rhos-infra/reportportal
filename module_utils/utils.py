...
def save_to_file(xml_doc, xml_path):
    ...
    with open(xml_path, 'wb') as xml_file:
        xml_file.write(etree.tostring(xml_doc, pretty_print=True))
...
def find_existing_path(path_pattern):
    ...
    # Initialize an empty list to store the existing path parts
    existing_path_parts = []

    for part in path_parts:
        # Join the existing path parts with "/" to form a potential path
        potential_path = "/".join(existing_path_parts + [part])

        # Check if the potential path exists
        if os.path.exists(potential_path):
            existing_path_parts.append(part)
        else:
            # If the potential path does not exist, stop checking further
            break

    # Join the existing path parts to form the existing path
